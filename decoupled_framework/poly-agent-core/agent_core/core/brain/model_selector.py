...
async def update_catalog(content: str, product_name: str = "General", doc_type: str = "product"):
    ...
    if not product_name or not str(product_name).strip():
        return "❌ update_catalog: El campo 'product_name' no puede estar vacío."
    if doc_type not in {"product", "policy", "company_info"}:
        return "❌ update_catalog: El campo 'doc_type' debe ser 'product', 'policy' o 'company_info'."

    doc_id = str(uuid.uuid5(uuid.NAMESPACE_URL, f"{doc_type}:{product_name.strip().lower()}"))
...
async def verify_fact(product_name: str):
    """
    Verifica el precio y stock REAL de un producto consultando Supabase.
    ...
    try:
        ...
        for row in rows:
            ...
            if product_name.lower() in name.lower():
                ...
                price_display = f"${price:,}" if isinstance(price, (int, float)) else "Consulte con Poly"
                return f"DATO OFICIAL (Supabase): {name} - Precio: {price_display}"
    except Exception:
        pass
...
def get_admin_tools():
    ...
    try:
        ...
        from core.brain.admin_tools import (
